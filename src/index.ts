#!/usr/bin/env node
/**
 * Gusto MCP Server — Exposed via Streamable HTTP
 *
 * Auth model: Client sends their own Gusto API access token as Bearer token.
...
app.get('/', (_req, res) => {
  ...
  res.json({
    ...
    version: '1.0.0',
    description:
      'Access Gusto Payroll & HR — employees, payrolls, benefits, contractors, time tracking through MCP tools.',
    mcpEndpoint: '/mcp',
    ...
    auth: {
      type: 'dual-mode',
      description:
        'Pass your Gusto API access token as the Bearer token. No credentials are stored on this server.',
      header: 'Authorization: Bearer <your-gusto-access-token>',
      howToGetKey:
        'Obtain an access token via the Gusto OAuth flow or from your Gusto developer dashboard.',
    },
    configTemplate: {
      mcpServers: {
        gusto: {
          ...
          headers: {
            Authorization: 'Bearer <your-gusto-access-token>',
          },
        },
      },
    },
...
<head>
  ...
  <title>Gusto MCP Server — AgenticLedger</title>
...
<body>
  ...
  <div class="container">
    <h1>Gusto MCP Server</h1>
    <p class="subtitle">Access Gusto Payroll & HR — employees, payrolls, benefits, contractors, time tracking through MCP tools.</p>

    <div class="card">
      <h2>How to get your Gusto API access token</h2>
      <ol class="steps">
        <li>Register your application at <a href="https://dev.gusto.com/" target="_blank" rel="noopener">Gusto Developer Portal</a>.</li>
        <li>Complete the <strong>OAuth 2.0</strong> authorization flow to obtain an access token.</li>
        <li>For sandbox testing, use the <strong>Gusto Demo</strong> environment.</li>
    ...
    <div class="card">
      ...
      <label for="apiKey">Gusto Access Token</label>
      <input type="text" id="apiKey" placeholder="Paste your access token here..." autocomplete="off" spellcheck="false" />
    ...
    <div class="footer">Powered by AgenticLedger &middot; ${tools.length} tools available</div>
  </div>
      ...
      return JSON.stringify({
        mcpServers: {
          gusto: {
            url: baseUrl + '/mcp',
            headers: {
              Authorization: 'Bearer ' + (key || 'YOUR_ACCESS_TOKEN')
            }
          }
        }
      }, null, 2);
...
function createMCPServer(client: GustoClient): Server {
...
app.post('/mcp', async (req, res) => {
  ...
  const client = new GustoClient(apiKey);
...
<head>
  ...
  <title>Authorize \u2014 Gusto MCP</title>
...
<body>
  <div class="card">
    <div class="header"><img src="/static/logo.png" alt="AgenticLedger"><span>Gusto MCP</span></div>
    <div class="consent-msg">An application wants to connect to <strong>Gusto MCP Server</strong> on your behalf. Enter your API key to authorize access.</div>
...
app.listen(PORT, () => {
  console.log(`Gusto MCP HTTP Server running on port ${PORT}`);
